...
//En C++, struct est un mot-clé utilisé pour définir une structure de données. Une structure de données est un type de données
//composé qui permet de regrouper plusieurs variables de types différents sous un même nom.

//Voici un exemple de déclaration d'une structure en C++ :
...
//float taille;
//};
//Dans cet exemple, nous avons déclaré une structure appelée Personne, qui contient trois variables membres :
//nom de type std::string, age de type int et taille de type float. Une fois déclarée, cette structure peut être utilisée pour créer des instances de Personne.
